...
@Version: v2.1
@Date: 2025-8-27
@Description: Translate unmatched English institution names using OpenAI, with a persistent loop to ensure all tasks are completed.
"""
...
def step_2_translate_unmatched(config: Config):
    ...
    try:
        ...
        required_cols = ['esid', 'data_source', 'nct_id', 'affiliation', 'state', 'city']
    ...
    # --- MODIFICATION START: 将核心逻辑放入 while 循环 ---
    processing_round = 1
    while not shutdown_event.is_set():
        logger.info("-" * 50)
        logger.info(f"开始第 {processing_round} 轮处理...")
        ...
        if not tasks:
            logger.info("所有任务均已成功处理，程序退出。")
            break  # 如果没有待处理任务，则跳出循环
            ...
            for result in llm_outputs:
                ...
                if result_id in batch_map:
                    ...
                    parsed_results.append({
                        ...
                        "trans_confidence": result.get('trans_confidence', ''),
                        "state": task.get('state'),
                        "city": task.get('city')
                    })

            llm_processed_ids = {res.get('esid') for res in parsed_results}
            for task in batch:
                task_id = task.get('esid')
                if task_id not in llm_processed_ids:
                    if is_chinese(task.get('affiliation')):
                        parsed_results.append({
                            ...
                            "trans_confidence": 'high', # 假设中文无需翻译，置信度高
                            "state": task.get('state'),
                            "city": task.get('city')
                        })
                    else:
        ...
        try:
            with tqdm(total=len(task_chunks), desc=f"翻译进度 (第 {processing_round} 轮)") as pbar:
    ...
    logger.info("所有处理循环已完成，开始最后清理...")
    cleanup_jsonl_file(config.TRANSLATED_JSONL_FILE, id_column='esid')
